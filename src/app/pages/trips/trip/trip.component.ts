...
export class TripComponent {
  ...
  private _calculatePaymentObligation(trip: ITrip) {
    const reducer = (payDto: IPayDto, student: IStudent) => {
      if (student.totalTripExpenses < trip.average) {
        let currentTotal = trip.average - student.totalTripExpenses;
        let paymentAmount = 0;
      ...
      else if (student.totalTripExpenses > trip.average) {
        let currentTotal = student.totalTripExpenses - trip.average;
        let debtAmount = 0;
